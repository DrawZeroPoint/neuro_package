      ...
      while (n.ok())
      {
        if(!trajectories.empty())
        {
          ...
          for(int i = 0; i<act_msg->goal.trajectory.joint_names.size();i++)
          {
            ...
            //traj.data[i]=act_msg->goal.trajectory.points[pos_count].positions[i];
          }
          ...
          do
          {
            ...
            for(int i = 0; i<act_msg->goal.trajectory.joint_names.size();i++)
            {
              command.positions[i].value = act_msg->goal.trajectory.points[pos_count-1].positions[i];
              between[i]=act_msg->goal.trajectory.points[pos_count].positions[i]-act_msg->goal.trajectory.points[pos_count-1].positions[i];
              each[i]=between[i]/k;
              //traj.data[i]=act_msg->goal.trajectory.points[pos_count].positions[i];
            }
              for(int j=0;j<k;j++)
